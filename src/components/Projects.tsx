...
const projects = [
  {
    ...
    description: 'Built a real-time crowd monitoring system combining object detection and multi-object tracking to identify congestion zones from live video feeds.',
  ...
  },
  {
    ...
    description: 'Developed a Power BI dashboard analyzing 10,000+ sales records with dynamic filters to track revenue, profit, and regional performance.',
  ...
  },
  {
    ...
    description: 'Designed and simulated a buck-boost converter for EV power applications, optimizing voltage stability and reducing output ripple.',
...
function ProjectCard({ project, index }: { project: typeof projects[0]; index: number }) {
  ...
  const CardContent = (
    <motion.div
      ref={ref}
      className="relative rounded-2xl overflow-hidden glass-effect-strong border border-white/10 hover:border-cyan-400/60 transition-all duration-500 shadow-xl hover:shadow-2xl flex flex-col"
    ...
    >
      {/* Image Container */}
      <div className="relative h-48 sm:h-56 overflow-hidden bg-gradient-to-br from-cyan-500/20 to-sky-500/20">
        <img
          src={project.image}
          alt={project.title}
      ...
      {/* Content */}
      <div className="relative z-10 p-6 flex flex-col flex-grow bg-black/40">
        <h3 className="text-xl sm:text-2xl font-bold text-cyan-400 group-hover:text-cyan-300 transition-colors mb-3">
          {project.title}
        </h3>
        ...
        <motion.button
          ...
          className="w-full px-4 py-3 bg-gradient-to-r from-cyan-600 to-sky-600 text-white font-semibold rounded-lg hover:from-cyan-500 hover:to-sky-500 transition-all duration-300 transform hover:shadow-lg hover:shadow-cyan-500/50 flex items-center justify-center gap-2"
  ...
  const OppositeContent = (
    ...
    >
      <h4 className={`text-lg font-bold text-white mb-4 flex items-center gap-3 ${isEven ? 'justify-start' : 'justify-end'}`}>
        {!isEven && <span className="w-12 h-px bg-cyan-500 hidden md:block"></span>}
        <span className="bg-gradient-to-r from-cyan-400 to-sky-400 bg-clip-text text-transparent">Project Highlights</span>
        {isEven && <span className="w-12 h-px bg-cyan-500 hidden md:block"></span>}
      </h4>
      <ul className={`space-y-4 ${isEven ? 'text-left pr-8' : 'text-right pl-8'}`}>
        {project.highlights.map((highlight, i) => (
          <li key={i} className="text-slate-400 text-sm sm:text-base leading-relaxed">
            {highlight}
          </li>
        ))}
      </ul>
  ...
  return (
    <div className="relative w-full mb-12 flex flex-col md:flex-row items-center justify-between group">
      ...
      <div className={`order-2 md:order-none w-full md:w-1/2 ${isEven ? 'md:pr-12 lg:pr-16' : 'md:pr-12 lg:pr-16'} pl-12 sm:pl-16 md:pl-0`}>
        {isEven ? CardContent : OppositeContent}
      </div>

      {/* Right Side (Info if Even, Card if Odd) */}
      <div className={`order-3 md:order-none w-full md:w-1/2 ${isEven ? 'md:pl-12 lg:pl-16' : 'md:pl-12 lg:pl-16'} hidden md:block`}>
